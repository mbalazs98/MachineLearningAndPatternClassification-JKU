...
class ClassifierOrchestrator:
    ...
    def get_final_prediction_for_one_instance_weighed(self, instance_prediction, weights):
        t=0
        f=0
		
        for i in instance_prediction[self.MUSIC_CLASSIFIERS_OUTPUT_INDEX]:
            if instance_prediction[self.MUSIC_CLASSIFIERS_OUTPUT_INDEX][i]==1:
                t=t+weights[self.MUSIC_CLASSIFIERS_OUTPUT_INDEX][i]
            else:
                f=f+weights[i]
        if t>f:
            music_final_output=1
        else:
            music_final_output=0
		
        t=0
        f=0
        for i in instance_prediction[self.SPEECH_CLASSIFIERS_OUTPUT_INDEX]:
            if instance_prediction[self.SPEECH_CLASSIFIERS_OUTPUT_INDEX][i]==1:
                t=t+weights[self.SPEECH_CLASSIFIERS_OUTPUT_INDEX][i]
            else:
                f=f+weights[self.SPEECH_CLASSIFIERS_OUTPUT_INDEX][i]
        if t>f:
            speech_final_output=1
        else:
            speech_final_output=0
    ...
    def get_final_bi_predictions(self, all_predictions, weights=None):
        ...
        """
        if not weights:
            weights=np.ones((len(all_predictions[MUSIC_CLASSIFIERS_OUTPUT_INDEX][0])),(len(all_predictions[SPEECH_CLASSIFIERS_OUTPUT_INDEX][0])))
